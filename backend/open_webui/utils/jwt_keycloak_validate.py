...
def validate_keycloak_jwt(token: str, public_key: str, audience: str, issuer: str) -> Optional[dict[str, any]]:
    try:
...
def getKeycloakPublicKey(keycloakURL: str) -> Optional[dict[str, any]]:
    ...
    try:
        ...
        print(f"Failed to parse refresh token response: {e}")
        return None
    return resp_json
...
def decodeAndValidateToken(token: str, issuer: str = '') -> Optional[dict[str, any]]:
    ...
    decodedAndVerifiedPayload = validate_keycloak_jwt(token, public_key, audience, issuer)
